...
internal class LineDrawingLayer : ScrollableGraphViewDrawingLayer {
    ...
    internal func createLinePath() -> UIBezierPath {
        ...
        let pathSegmentAdder: (CGPoint, CGPoint, UIBezierPath) -> ()
        
        switch lineStyle {
        case .smooth:
            pathSegmentAdder = addCurvedLineSegment
        case .straight:
            pathSegmentAdder = addStraightLineSegment
        case .stepped:
            pathSegmentAdder = addSteppedLineSegment
        }
        
//        let pathSegmentAdder = lineStyle == .smooth ? addCurvedLineSegment : addStraightLineSegment
    ...
    private func addSteppedLineSegment(startPoint: CGPoint, endPoint: CGPoint, inPath path: UIBezierPath) {
